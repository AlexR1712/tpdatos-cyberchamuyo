...
int NodoInterno::insertarRecursivo(Registro* r) {
	...
	if((*nodo_data)[0] == 'H') {
		...
		if(nodo_interno->tieneOverflow()) {
			...
			int nuevo_ni_derecho_id = arbol->crearNodo();
...
int NodoInterno::encontrarPrimero() {
	int hijo_izq = nodos[0];
	if(nivel == 1) {
		return hijo_izq;
	} else {
		int pos = 0;
		std::vector<char>* nodo_data = arbol->leerNodo2(hijo_izq);
		NodoInterno* nI = new NodoInterno(nivel - 1, arbol);
		nI->hidratar(nodo_data, pos);
		int prim = nI->encontrarPrimero();
		delete nodo_data;
		delete nI;
		return prim;
	}
